...
RestWrite.prototype.handleOAuthAuthData = function (provider) {
  ...
  return validateAuthData(authData, oauthOptions).then(function () {
    ...
    return _this4.config.database.find(_this4.className, query, {});
  }).then(function (results) {
    _this4.storage['authProvider'] = provider;
    if (results.length > 0) {
      if (!_this4.query) {
        // We're signing up, but this user already exists. Short-circuit
        delete results[0].password;
      ...
      _this4.data.username = cryptoUtils.newToken();
    }

    // This FB auth does not already exist, so transform it to a
    // saveable format
    _this4.data["_auth_data_" + provider] = authData;
...
RestWrite.prototype.transformUser = function () {
  ...
  return promise.then(function () {
    ...
    if (_this5.query && !_this5.auth.isMaster) {
    ...
    if (!_this5.data.username) {
      if (!_this5.query) {
        _this5.data.username = cryptoUtils.randomString(25);
      }
      return;
    }
