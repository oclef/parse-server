...
function classNameIsValid(className) {
  ...
  className === '_Role' || joinClassRegex.test(className) ||
...
Schema.prototype.deleteField = function (fieldName, className, database, prefix) {
  ...
  return this.reload().then(function (schema) {
    return schema.hasClass(className).then(function (hasClass) {
      if (!hasClass) {
        return Promise.reject({
          code: Parse.Error.INVALID_CLASS_NAME,
      ...
      if (schema.data[className][fieldName].startsWith('relation')) {
        ...
        .then(function () {
          return _this4.collection.update({ _id: className }, { $unset: _defineProperty({}, fieldName, null) });
        });
      } else {
        ...
        return new Promise(function (resolve, reject) {
          database.collection(prefix + className, function (err, coll) {
            if (err) {
              reject(err);
            } else {
              var mongoFieldName = schema.data[className][fieldName].startsWith('*') ? '_p_' + fieldName : fieldName;
              return coll.update({}, {
                "$unset": _defineProperty({}, mongoFieldName, null)
              }, {
                multi: true
              })
              //Save the _SCHEMA object
...
Schema.prototype.hasClass = function (className) {
  return this.reload().then(function (newSchema) {
    return !!newSchema.data[className];
  });
};
